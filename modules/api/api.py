...
from modules.translator_api import translate_text_api
...
class Api:
    def __init__(self, app: FastAPI, queue_lock: Lock):
        ...
        self.add_api_route("/sdapi/v1/translate",self.translate, methods=["POST"])
    ...
    def set_config(self, req: dict[str, Any]):
        ...
        set_config(req)
    def translate(self, req: dict[str, Any]):
        text = req.get("prompt")
        source_lang = req.get("source_language")
        target_lang = req.get("target_language", "English")
    
        if not text or not source_lang:
            return JSONResponse(status_code=400, content={"error": "Missing text or source_language"})

        try:
            translated = translate_text_api(text, source_lang, target_lang)
